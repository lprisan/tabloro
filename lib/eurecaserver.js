...
module.exports = function (eurecaServer) {
    ...
    function saveDesign (room) {
        console.log('saving design', room.name);
        Table.eurecaCopyUpdateDesign(room.name, room.tiles, room.stacks, room.rawchilitags, room.regions);
    }
    ...
    eurecaServer.exports.handshake = function (newClientId, cursorId, name, roomId, mode) {
        ...
        findOrCreateRoom(roomId, mode, function (room) {
            ...
            newClientRemote.receiveChat(currentClient(that), 'Hit ENTER to chat');
    ...
    eurecaServer.exports.tilesDragStart = function (selected, relativePositions) {
        log('tilesDragStart');
        var that = this;
    ...
    // be exposed to client side
    eurecaServer.exports.tilesDragStop = function (selected, newPositions) {
        log('tilesDragStop', selected, newPositions);
        var that = this;
        ...
        currentRoom(that, function (room) {
            ...
            R.forEach.idx(function (tileId, index) {
                ...
                room.tiles[tileId] = R.mixin(room.tiles[tileId], newPosition);
    ...
    eurecaServer.exports.flipTiles = function (selected, frames) {
     ...
     eurecaServer.exports.tileLock = function (tileId) {
        log('tileLock', tileId);
        var that = this;

        currentRoom(that, function (room) {
            if (room.tiles[tileId].lock) {
                delete room.tiles[tileId].lock;
                forEachClient(room, function (remote) {
                    remote.unlock(currentClient(that), tileId);
                });
            } else {
                room.tiles[tileId].lock = true;
                forEachClient(room, function (remote) {
                    remote.lock(currentClient(that), tileId);
                });
            }
            saveTable(room);
        });
     ...
     eurecaServer.exports.tileOwnedBy = function (tileId) {
        log('tileOwnedBy', tileId);
        var that = this;
        var clientName = currentClient(that).name;

        currentRoom(that, function (room) {
            if (!room.tiles[tileId].ownedBy) {
                room.tiles[tileId].ownedBy = clientName;
                forEachClient(room, function (remote) {
                    remote.ownedBy(currentClient(that), tileId);
                });

            } else if (room.tiles[tileId].ownedBy === clientName) {
                delete room.tiles[tileId].ownedBy; // release
                forEachClient(room, function (remote) {
                    remote.releasedBy(clientName, tileId);
                });
            } else {
                return; // forbidden
            }
    ...
    eurecaServer.exports.shuffleStack = function (tiles, position) {
        var that = this;
        ...
        currentRoom(that, function (room) {
            ...
            forEachClient(room, function (remote) {
                remote.updateStackCards(currentClient(that), 'shuffle', shuffledTiles, position);
            });
        });
    };


    eurecaServer.exports.tidyStack = function (tiles, position) {
        var that = this;

        currentRoom(that, function (room) {
            log('tidy', tiles, 'in room', room.name);

            forEachClient(room, function (remote) {
                remote.updateStackCards(currentClient(that), 'tidy', tiles, position);
            });
        });
    };
    ...
    eurecaServer.exports.saveDesign = function (rawchilitags, boardRegions) {
        log('saveDesign');
        var that = this;

        currentRoom(that, function (room) {

            room.rawchilitags = rawchilitags;
            room.regions = boardRegions;
            saveDesign(room);
        });
    ...
    eurecaServer.exports.arrangeLayer = function (name) {
        log('arrangeLayer');
        var that = this;

        currentRoom(that, function (room) {
            forEachClient(room, function (remote) {
                remote.arrangeLayer(currentClient(that), name);
            });
        });
